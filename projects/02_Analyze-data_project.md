...
ebay_data_pre_2014 = ebay_data[ebay_data['date'] < pd.to_datetime('2014-03-01')]
plt.plot(ebay_data_pre_2014['date'], ebay_data_pre_2014['open'])
plt.xlabel('date')
plt.ylabel('opening price (\$)');
```

```{code-cell} ipython3
diffprice = np.diff(ebay_data_pre_2014['open'])
plt.plot(ebay_data_pre_2014['date'][1:], diffprice)
plt.xlabel('date')
plt.ylabel('change in opening price (\$/day)');
```

```{code-cell} ipython3
mean_diffprice = np.mean(diffprice)
std_diffprice = np.std(diffprice)
x_ebay = np.linspace(-10, 10)
from scipy import stats
price_pdf = stats.norm.pdf(x_ebay, loc = mean_diffprice, scale = std_diffprice)

print(mean_diffprice)
print(std_diffprice)
```

```{code-cell} ipython3
plt.hist(diffprice, 50, density=True)
plt.plot(x_ebay, price_pdf)
plt.title('EBAY changes in price over 4 years\n'+
         'avg: \${:.2f} stdev: \${:.2f}'.format(mean_diffprice, std_diffprice));
```

```{code-cell} ipython3
rng = default_rng(42)
N_models = 100
diffprice_model = rng.normal(size = (len(ebay_data_pre_2014), N_models), loc = 0.03, scale = 0.71)

plt.hist(diffprice, 50, density=True, label = 'NYSE data')
plt.plot(x_ebay, price_pdf)
plt.hist(diffprice_model[:, 0], 50, density = True, 
         histtype = 'step', 
         linewidth = 3, label = 'model prediction 1')
plt.title('EBAY changes in price over 4 years\n'+
         'avg: \${:.2f} stdev: \${:.2f}'.format(mean_diffprice, std_diffprice))
...
```{code-cell} ipython3
ebay_price_model = np.cumsum(diffprice_model, axis = 0) + ebay_data_pre_2014['open'].values[0]

plt.plot(ebay_data_pre_2014['date'], ebay_price_model, alpha = 0.3);

plt.plot(ebay_data_pre_2014['date'], ebay_data_pre_2014['open'], c = 'k', label = 'NYSE data')
plt.xlabel('date')
plt.ylabel('opening price (\$)');
```

```{code-cell} ipython3
price_model_avg_ebay = np.mean(ebay_price_model, axis = 1)
price_model_std_ebay = np.std(ebay_price_model, axis = 1)
...
skip = 100
plt.errorbar(ebay_data_pre_2014['date'][::skip], price_model_avg_ebay[::skip],
             yerr = price_model_std_ebay[::skip], 
             fmt = 'o',
             c = 'r', 
             label = 'model result', 
            zorder = 3);
